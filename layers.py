...
def encode (in_data, training):
    '''从原图获取特征'''
    l1 = tf.layers.conv2d(in_data, ngf*2, 3, 2, 'same', activation=lReLU) # 128x128x64
    l2 = tf.layers.conv2d(l1, ngf*4, 3, 2, 'same', activation=lReLU) # 64x64x128
    l3 = tf.layers.conv2d(l2, ngf*8, 2, 1, 'same', activation=lReLU, use_bias=False) # 64x64x256
    out = tf.layers.batch_normalization(l3, training=training)
    return out

def rn_block (in_data, num_features):
    ''' build resnet block'''
    l1 = tf.layers.conv2d(in_data, num_features, 3, 1, 'same', activation=lReLU)
...
def decode (in_data, training):
    '''将变换后的特征复原到原图'''
    l1 = tf.layers.conv2d_transpose(in_data, ngf*2, 3, 2, 'same', activation=lReLU)
    l2 = tf.layers.conv2d_transpose(l1, ngf, 3, 2, 'same', activation=lReLU)
    l3 = tf.layers.conv2d(l2, 3, 2, 1, 'same', activation=lReLU, use_bias=False)
